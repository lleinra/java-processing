...
void setup() {
  ...
  rows = int(values[values.length - 1]);
  setup(true);
}

void setup(Boolean s) {
...
Boolean inConst(Cell curr, Boolean checkingCol) {
  groups.clear();
...
Boolean verifyRow(Cell curr, Boolean checkingCol) {
  groups.clear();
  Cell[] col = new Cell[rows];
...
void preset() {
  println(values.length / 2, int(values[values.length - 1]));
  if (rows != int(values[values.length - 1])) {
    return;
  }
